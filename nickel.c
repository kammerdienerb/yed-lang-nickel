...
LOG_FN_ENTER();
    ...
    if (yed_plugin_make_ft(self, "Nickel") == FT_ERR_TAKEN) {
        yed_cerr("lang/nickel: unable to create file type name");
        LOG_EXIT();
        return 1;
    }
    ...
    yed_set_var("Nickel-comment-style", ";;");
...
void maybe_change_ft(yed_buffer *buff) {
    ...
    if (strcmp(ext, "nickel") == 0) {
        yed_buffer_set_ft(buff, yed_get_ft("Nickel"));
    }
}
